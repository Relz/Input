...
TEST(read_vector_with_true_char_function, returns_false_if_can_not_read_any_element)
{
	...
	EXPECT_FALSE(input.ReadVector(booleans, VectorSettingsBuilder<bool>().SetTrueChar('#').Build()));
}
...
TEST(read_vector_with_true_char_function, push_back_by_default)
{
	...
	Input input(is);
	vector<bool> booleans;
	EXPECT_TRUE(input.ReadVector(booleans, VectorSettingsBuilder<bool>().SetTrueChar('#').Build()));
...
TEST(read_vector_with_true_char_function, can_push_front)
{
	...
	EXPECT_TRUE(input.ReadVector(
		...
		VectorSettingsBuilder<bool>().SetTrueChar('#').Build(
			BaseSettingsBuilder().SetReadMethod(ReadVectorMethod::PUSH_FRONT).Build())));
	EXPECT_EQ(booleans, expectedVector);
}

TEST(read_vector_with_true_char_function, skip_whitespaces_and_empty_lines)
{
	vector<bool> expectedVector{ false, false, false, false, false, false, false, false, false,
...
TEST(read_vector_with_true_char_function, can_have_skip_characters)
{
	...
	EXPECT_TRUE(input.ReadVector(
		booleans,
		VectorSettingsBuilder<bool>().SetTrueChar('#').Build(
			BaseSettingsBuilder().SetSkipCharacters({ ' ', '\n' }).Build())));
	EXPECT_EQ(booleans, expectedVector);
}
...
TEST(read_vector_with_true_char_function, can_have_stop_characters)
{
	...
	EXPECT_TRUE(input.ReadVector(
		booleans,
		VectorSettingsBuilder<bool>().SetTrueChar('#').Build(BaseSettingsBuilder().SetStopCharacters({ '.' }).Build())));
	EXPECT_EQ(booleans, expectedVector);
}
...
TEST(read_vector_with_true_char_function, can_have_limit)
{
	...
	vector<bool> booleans;
	EXPECT_TRUE(input.ReadVector(
		booleans, VectorSettingsBuilder<bool>().SetTrueChar('#').Build(BaseSettingsBuilder().SetReadLimit(3).Build())));
	EXPECT_EQ(booleans, expectedVector);
}
