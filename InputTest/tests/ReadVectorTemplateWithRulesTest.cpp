...
TEST(read_vector_template_with_rules, returns_false_if_can_not_read_any_element)
{
	...
	EXPECT_FALSE(input.ReadVector(booleans, VectorSettingsBuilder<bool>().SetRules(RULES_BOOLEANS).Build()));
}
...
TEST(read_vector_template_with_rules, returns_false_if_can_not_convert_any_element)
{
	...
	}
	{
		...
		EXPECT_FALSE(input.ReadVector<char>(numbers, VectorSettingsBuilder<size_t>().SetRules(RULES_NUMBERS).Build()));
	...
	}
	{
		...
		EXPECT_FALSE(input.ReadVector(characters, VectorSettingsBuilder<char>().SetRules(RULES_CHARACTERS).Build()));
...
TEST(read_vector_template_with_rules, push_back_by_default)
{
	...
	Input input(is);
	vector<bool> booleans;
	EXPECT_TRUE(input.ReadVector(booleans, VectorSettingsBuilder<bool>().SetRules(RULES_BOOLEANS).Build()));
	EXPECT_EQ(booleans, expectedVector);
}
...
TEST(read_vector_template_with_rules, can_have_limit)
{
	...
	EXPECT_TRUE(input.ReadVector(
		booleans,
		VectorSettingsBuilder<bool>().SetRules(RULES_BOOLEANS).Build(BaseSettingsBuilder().SetReadLimit(4).Build())));
	EXPECT_EQ(booleans, expectedVector);
}
