...
TEST(read_vector_char, can_push_front)
{
	...
	EXPECT_TRUE(input.ReadVector(
		chars,
		VectorSettingsBuilder<char>().Build(BaseSettingsBuilder().SetReadMethod(ReadVectorMethod::PUSH_FRONT).Build())));
	EXPECT_EQ(chars, expectedVector);
}
...
TEST(read_vector_char, skips_characters_before_reading_first_element)
{
	...
	vector<char> chars;
	EXPECT_TRUE(input.ReadVector(
		chars, VectorSettingsBuilder<char>().Build(BaseSettingsBuilder().SetSkipCharacters({ ' ', '\n' }).Build())));
	EXPECT_EQ(chars, expectedVector);
}
...
TEST(read_vector_char, can_have_limit)
{
	...
	vector<char> chars;
	EXPECT_TRUE(
		input.ReadVector(chars, VectorSettingsBuilder<char>().Build(BaseSettingsBuilder().SetReadLimit(3).Build())));
	EXPECT_EQ(chars, expectedVector);
}
