...
TEST(read_matrix_template, can_have_limit)
{
	...
	Input input(is);
	vector<vector<int>> matrix;
	EXPECT_TRUE(input.ReadMatrix(matrix, MatrixSettingsBuilder().Build(BaseSettingsBuilder().SetReadLimit(2).Build())));
	EXPECT_EQ(matrix, expectedMatrix);
}
...
TEST(read_matrix_template, can_have_stop_characters)
{
	...
	vector<vector<int>> matrix;
	EXPECT_TRUE(input.ReadMatrix(
		matrix, MatrixSettingsBuilder().Build(BaseSettingsBuilder().SetStopCharacters({ '#' }).Build())));
	EXPECT_EQ(matrix, expectedMatrix);
}
