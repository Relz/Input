...
TEST(skip_until_symbols_function, skips_until_symbols_and_returns_true_if_any_symbol_was_reached)
{
	...
	}
	{
		...
		CInput input(is);
		std::string skippedString;
		EXPECT_FALSE(input.SkipUntilCharacters({ SPACE_SYMBOL }, skippedString));
		EXPECT_TRUE(is.eof());
		EXPECT_EQ(input.GetPosition().GetLine(), 1);
...
{
	{
		...
		CInput input(is);
		std::string skippedString;
		EXPECT_TRUE(input.SkipUntilCharacters({ 'a', ENDL_SYMBOL }, skippedString));
		EXPECT_EQ(is.peek(), char_traits<char>::to_int_type('\n'));
		EXPECT_EQ(input.GetPosition().GetLine(), 1);
	...
	}
	{
		...
		CInput input(is);
		std::string skippedString;
		EXPECT_FALSE(input.SkipUntilCharacters({ SPACE_SYMBOL, ENDL_SYMBOL }, skippedString));
		EXPECT_TRUE(is.eof());
		EXPECT_EQ(input.GetPosition().GetLine(), 1);
		EXPECT_EQ(input.GetPosition().GetColumn(), 2);
		EXPECT_EQ(skippedString, "#");
